...
interface RideCreationStore {
  ...
  setVehicleCategoryId: (vehicleCategoryId: number) => void;
...
export const useRideCreationStore = create<RideCreationStore>()(
  persist<RideCreationStore>(
    (set, get) => ({
      ...
      setRideRoute: (ride_route) =>
        set((state) => ({
          rideData: { ...state.rideData, ride_route },
        })),

      setSelectedRoutePolyline: (selected_route_polyline) =>
        set((state) => ({
      ...
      setDepartureDate: (departure_date) => {
        ...
        set((state) => {
          console.log(
            "Setting departure_date from:",
            state.rideData.departure_date,
            "to:",
            departure_date
          );
          return {
      ...
      setMax2InBack: (max_2_in_back) =>
        set((state) => ({
          rideData: { ...state.rideData, max_2_in_back },
        })),

      setPricePerSeat: (price_per_seat) =>
        set((state) => ({
      ...
      setVehicleCategoryId: (vehicle_category_id) =>
        set((state) => ({
          rideData: { ...state.rideData, vehicle_category_id },
        })),

      setIsReturn: (is_return) =>
        set((state) => ({
